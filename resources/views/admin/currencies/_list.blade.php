		...
		@foreach ($currencies as $key => $item)
			<tr>
				<td width="1%">
					<div class="text-nowrap">
						<div class="checkbox inline">
							<label>
								<input type="checkbox" class="node styled" name="ids[]" value="{{ $item->uid }}" />
							</label>
						</div>
					</div>
				</td>
				...
				<td class="text-right text-nowrap" width="5%">
					@can('update', $item)
						<a href="{{ action('Admin\CurrencyController@edit', $item->uid) }}" data-popup="tooltip" title="{{ trans('messages.edit') }}" type="button" class="btn btn-secondary btn-icon"><i class="icon icon-pencil pr-0 mr-0"></i></a>
					@endcan
